...
/** Internal use only.  Create a default configuration. */
+ (nullable IXNMuseConfiguration *)makeDefaultConfiguration:(IXNMuseModel)model;

/**
 * Internal use only. Create configuration from a JSON string.  Muse 2016 (
 * \if ANDROID_ONLY
 ...
 * ) only.
 */
+ (nullable IXNMuseConfiguration *)makeConfiguration:(nonnull NSString *)json;

/** Provides access to current preset. */
- (IXNMusePreset)getPreset;

/**
 * The name of the headband. This is only available on %Muse 2016 (
 * \if ANDROID_ONLY
 * \link com.choosemuse.libmuse.MuseModel.MU_02 MU_02\endlink
 * \elseif IOS_ONLY
 * \link ::IXNMuseModel::IXNMuseModelMu02 MU_02\endlink
